    ...
    data.entry.forEach(function(pageEntry) {
      ...
      pageEntry.messaging.forEach(function(messagingEvent) {
        ...
        if (messagingEvent.optin) {
          ...
          utils.receivedMessage(messagingEvent, req.session, userModel);
        } else if (messagingEvent.delivery) {
          utils.receivedDeliveryConfirmation(messagingEvent);
        } else if (messagingEvent.postback) {
          utils.receivedPostback(messagingEvent, req.session, userModel);
        } else if (messagingEvent.account_linking) {
          utils.receivedAccountLinking(messagingEvent, req.session, userModel);
        } else {
          ...
          api.getUserId(obInternalToken, userName).then(function(resp) {
            ...
            api.enableUser(obInternalToken, userId).then(function(resp) {
              api.getAmplifyToken(userName, userPassword).then(function(resp) {
                ...
                userModel.setUserProperties(senderID, prop).then(function(user) {
                  userModel.setUserStoredPassword(senderID, userPassword).then(function(user) {
