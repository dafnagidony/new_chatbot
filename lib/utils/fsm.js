...
var fsm = fsm.StateMachine.create({
  ...
  events: [
    ...
    {name: 'adjustBudgetEvent', from: ['manageCampaign', 'changeConfirmed', 'changeIncorrect'], to: 'adjustBudget'},
    {name: 'adjustCpcEvent', from: ['manageCampaign', 'changeConfirmed', 'changeIncorrect'], to: 'adjustCpc'},
...
callbacks: {
    ongetStarted: function(event, from, to, sessionObj, userModel) {  
      ...
      console.log('------------------ from onlogin');
	   	var elements = {   
		    title: "Hi " + name + ",", //resp.first_name
    ...
    onselectAccount: function(event, from, to, senderID) {
    ...
    onconfirmed: function(event, from, to, senderID, action) {
      ...
      quick_replies = quick_replies.concat(replies); 
      }
    ...
    onselectCampaign: function(event, from, to, senderID) {
    	var obToken = cache.get('obToken');
		  var accountObj = cache.get('accountLookup');
      var account = cache.get('account');
    ...
    onmanageCampaign: function(event, from, to, senderID) {    
      utils.getCampaignInformation(senderID).then(function(campaignData) {
        ...
        cache.put('enabled', campaignData.enabled);
        var elements = {   
    ...
    onviewCampaign: function(event, from, to, senderID) {
      var quick_replies = [
          {
            content_type:"text",
            title: 'Today',
    ...
    onadjustCpc: function(event, from, to, senderID) {
      utils.getCampaignInformation(senderID).then(function(campaignData) {
    ...
    onamountEntered: function(event, from, to, senderID, amount) {
      var action = (from == "adjustBudget") ? "Budget" : "CPC";
      var originalValue = (from == "adjustBudget")? cache.get('budget') : cache.get('cpc');
      var elements = {   
      ...
      if (action == "Budget") {
        ...
        cache.put('updatedBudget', amount);
      } else {
        elements.buttons[0].payload = "cpcAmountCorrect";
        elements.buttons[1].payload = "cpcAmountNotCorrect";
        cache.put('updatedCpc', amount);
      }
      messages.sendGenericMessage(senderID, elements);
    },
    onchangeConfirmed: function(event, from, to, senderID, action) {
      obToken = cache.get('obToken');
      utils.getCampaignInformation(senderID).then(function(campaignData) {
        var budgetId = campaignData.budget.id;
        if (action == 'budget') {
          var amount = cache.get('updatedBudget');
          ...
          var amount = cache.get('updatedCpc');
