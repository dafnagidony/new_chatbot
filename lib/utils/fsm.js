...
var fsm = fsm.StateMachine.create({
  ...
  events: [
    ...
    {name: 'selectAccountEvent', from: '*', to: 'selectAccount'}, //['getStarted', 'login', 'chosen', 'confirmed']
    {name: 'selectCampaignEvent', from: '*', to: 'selectCampaign'}, //'confirmed'
...
callbacks: {
    ongetStarted: function(event, from, to, sessionObj, userModel) {  
      var senderID = sessionObj.senderID;	 	
        ...
        userModel.getBySenderID(senderID).then(function(user) {
          ...
          if (!user) {
            ...
            userModel.add(userProp).then(function(user) {
              ...
              fsm.login(sessionObj, resp.first_name);
            });
          }
          else {
            if (user.authenticated && user.name !== '') {
              ...
              messages.sendGenericMessage(senderID, elements)
                .then(function() {
                  fsm.selectAccountEvent(sessionObj, userModel);
                }); 
            } else {
              fsm.login(sessionObj, resp.first_name);
            }
 		...
 		onlogin: function(event, from, to, sessionObj, name) {
      senderID = sessionObj.senderID;
	   	var elements = {   
		    title: "Hi " + name + ",",
    ...
    onselectAccount: function(event, from, to, sessionObj, userModel) {
      var senderID = sessionObj.senderID;
      userModel.getBySenderID(senderID).then(function(user) {
        ...
        api.getMarketers(obToken).then(function(marketers) {
          ...
          sessionObj.accountLookup = accountObj;
          if (recentAccounts) {
    ...
    onchosen: function(event, from, to, sessionObj, name) {
      var senderID = sessionObj.senderID;
      var elements = {   
    ...
    onconfirmed: function(event, from, to, sessionObj, action) {
      var senderID = sessionObj.senderID;
      var campaign = sessionObj.campaign;
    ...
    onselectCampaign: function(event, from, to, sessionObj, userModel) {
      var senderID = sessionObj.senderID;
      userModel.getBySenderID(senderID).then(function(user) {
        var obToken = user.obToken;
  		  var accountObj = sessionObj.accountLookup;
        var account = sessionObj.account;
          ...
          var recentCampaigns = cache.get('mostUsedCampaigns');
            ...
            sessionObj.campaignLookup = campaignObj;
            if (recentCampaigns) {
    ...
    onmanageCampaign: function(event, from, to, sessionObj, userModel) {
      var senderID = sessionObj.senderID;    
      utils.getCampaignInformation(sessionObj, userModel).then(function(campaignData) {
        var campaignAction = campaignData.enabled ? 'Stop Campaign' : 'Start Campaign';
        sessionObj.campaignEnabled = campaignData.enabled;
        var elements = {   
    ...
    onviewCampaign: function(event, from, to, sessionObj, userModel) {
      var quick_replies = [
          {
            content_type:"text",
            title: 'Today',
    ...
    onadjustCpc: function(event, from, to, sessionObj) {
      var senderID = sessionObj.senderID;
      utils.getCampaignInformation(senderID).then(function(campaignData) {
    ...
    onturnOnOffEvent: function(event, from, to, sessionObj) {
      ...
      utils.getCampaignInformation(senderID).then(function(campaignData) {
        ...
        .then(function() {
          ...
          messages.sendTextMessage(senderID, message).then(function() {
            utils.whatToDoNextMessage(senderID);
          });
        })
        .catch(function(err) {
    ...
    onamountEntered: function(event, from, to, sessionObj, amount) {
      var senderID = sessionObj.senderID;
      var action = (from == "adjustBudget") ? "Budget" : "CPC";
      var originalValue = (from == "adjustBudget")? sessionObj.budget : sessionObj.cpc;
      var elements = {   
      ...
      if (action == "Budget") {
        elements.buttons[0].payload = "budgetAmountCorrect";
        elements.buttons[1].payload = "budgetAmountNotCorrect";
        sessionObj.updatedBudget = amount;
      } else {
        elements.buttons[0].payload = "cpcAmountCorrect";
        elements.buttons[1].payload = "cpcAmountNotCorrect";
        sessionObj.updatedCpc = amount;
      }
      messages.sendGenericMessage(senderID, elements);
    },
    onchangeConfirmed: function(event, from, to, sessionObj,userModel, action) {
      userModel.getBySenderID(senderID).then(function(user) {
        var obToken = user.obToken;
        var senderID = sessionObj.senderID;
        utils.getCampaignInformation(senderID).then(function(campaignData) {
          ...
          if (action == 'budget') {
            var amount = sessionObj.updatedBudget;
            ...
            .catch(function(err) {
              fsm.changeIncorrectEvent(sessionObj, 'budget');
            });
          } else { 
            var amount = sessionObj.updatedCpc;
            ...
            .catch(function(err) {
              ...
              fsm.changeIncorrectEvent(sessionObj, 'cpc');
            });
          }
        });
      });
    },
    onchangeIncorrect: function(event, from, to, sessionObj, action) {
      var senderID = sessionObj.senderID;
      console.log('%%%%----- from change Incorrect', from, action);
      messages.sendTextMessage(senderID, "Whoops. Let's try again.").then(function() {
        if (action == 'budget') {
          fsm.adjustBudgetEvent(sessionObj);
        } else if (action == 'cpc') {
          fsm.adjustCpcEvent(sessionObj);
        }    
      });
    }
  }
