...
module.exports.receivedMessage = function(event, sessionObj, userModel){
  ...
  if (message.quick_reply) {
    ...
    if (message.quick_reply.payload.search("accountSelect") !== -1) {
      ...
      console.log(' ########## ~~~~~~account is selected  ', sessionObj);
      fsm.chosenEvent(sessionObj, account);
    } else if (message.quick_reply.payload.search("campaignSelect") !== -1) {
      var start = messagePayload.search("-")+1;
      var campaign = messagePayload.slice(start);
      sessionObj.campaign = campaign;
      fsm.chosenEvent(sessionObj, campaign);
    ...
    if (options.indexOf(fsm.current) !== -1  && !isNaN(message.text)) {
      fsm.amountEnteredEvent(sessionObj, message.text);
    }
    else {
      console.log('<<<<<<< NLP >>>>>>>>>>');
      nlp.nlpResponse(message.text, sessionObj, userModel);
    }
  }
}
...
module.exports.receivedPostback = function(event, sessionObj, userModel) {
  ...
  if (action == 'get_started') {
    ...
    console.log('################## accountIsCorrect  ', sessionObj);
    fsm.confirmedEvent(sessionObj, 'account');
  }
  else if (action === 'campaignIsCorrect') {
    fsm.confirmedEvent(sessionObj, 'campaign');
  }
    ...
    fsm.turnOnOffEvent(sessionObj);
  } 
  else if (action === 'budgetAmountCorrect') {
    fsm.changeConfirmedEvent(sessionObj,userModel, 'budget');
  }
  else if (action === 'cpcAmountCorrect') {
    fsm.changeConfirmedEvent(sessionObj,userModel, 'cpc');
  }
...
module.exports.receivedAccountLinking = function(event, sessionObj, userModel) {
...
function getCampaignInformation(sessionObj, userModel) {
  var senderID = sessionObj.senderID;  
  return userModel.getBySenderID(senderID).then(function(user) {
    ...
    return api.getCampaign(obToken, campaignId).then(function(campaign) {
      console.log('******** from getCampaignInformation: ', campaign);
      return campaign;
    });
  });
}
