 ...
 * @returns {Object} Promise fulfilled with User object
 */
User.prototype.setUserStoredPassword = function setUserStoredPassword(userName, password) {
  var self = this;
  ...
  return utils.core.promise(function(resolve, reject) {
    bcrypt.genSalt(10, function(err, salt) {
      ...
      return bcrypt.hash(password, salt, function(error, hash) {
        ...
        self.setUserProperties(userName, [{salty_password: hash}]).then(function(savedUser) {
          return resolve(savedUser);
        })
        .catch(function(err) {
          return reject(err);
